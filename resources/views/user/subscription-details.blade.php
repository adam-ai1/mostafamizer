    ...
    <div class="w-[68.9%] 6xl:w-[85.9%] dark:bg-[#292929] flex flex-col flex-1 border-l dark:border-[#474746] border-color-DF subscription-details border-right">
        <div class="xl:flex w-full h-full subscription-main md:overflow-auto sidebar-scrollbar md:h-screen">
            ...
            @if (!subscription('getUserSubscription', auth()->user()->id))
                <div class="grow 2xl:pl-6 8xl:pr-[84px] px-5 xl:pt-[74px] md:pt-5 pt-[74px] pb-[46px] dark:bg-[#292929] xl:overflow-auto sidebar-scrollbar main-profile-content md:h-screen xl:w-1/2">
                    <a class="flex justify-start items-center font-Figtree text-color-14 dark:text-white text-15 font-normal gap-2.5 md:hidden xl:px-6 pb-4 profile-back cursor-pointer">
                    ...
                    <section class="show-all-plans hidden mt-6">
                        <p class="font-semibold text-color-14 dark:text-white text-20 pb-3">{{ __('Plans that suits your needs and budget.') }}</p>
                        <p class="font-medium text-color-14 dark:text-white text-15 pb-3">{{ !empty($billingCycles) ? __('We have plans for everyone, whether you are a solo marketer or a large agency. 100% secured payment and no hidden fees.') : __('At the moment, subscription plans are not available. Please await further instructions from the administrator regarding subscription options.') }}</p>
                        @if (!empty($billingCycles))
                            <div class="mt-5">
                                @include('user.includes.billing-cycle-tabs')
                                @include('user.includes.plans-container')
                            </div>
                        @endif
                    </section>
                    <section class="show-current-subscription">
                        <div class="mt-6 flex 8xl:flex-row flex-col justify-between gap-4 w-full">
                            ...
                            <div class="bg-color-F6 dark:bg-color-3A lg:p-6 p-4 rounded-xl 8xl:w-[66.5%] subscription-profile-card h-[220px] w-full flex flex-col justify-between">
                                <div>
                        ...
                        @include('user.includes.billing-info')
                        @include('user.includes.cancel-subscription')
                    </section>
                </div>
                                                ...
                                                <div class="upgradePlan-modal-container">
                                                    ...
                                                    <div class="mt-6 mb-7">
                                                        ...
                                                        <div class="flex flex-col gap-4 mt-6 sub-modal-rtl h-80 pr-6 overflow-auto sidebar-scrollbar">
                                                            @php
                                                                $activeIconSvg = '<svg class="neg-transition-scale" xmlns="http://www.w3.org/2000/svg" width="12" height="9" viewBox="0 0 12 9" fill="none"><path d="M11.1433 1.10826C11.4609 1.42579 11.4609 1.94146 11.1433 2.25899L4.64036 8.76197C4.32283 9.0795 3.80717 9.0795 3.48964 8.76197L0.238146 5.51048C-0.0793821 5.19295 -0.0793821 4.67728 0.238146 4.35976C0.555675 4.04223 1.07134 4.04223 1.38887 4.35976L4.06627 7.03462L9.99516 1.10826C10.3127 0.790735 10.8284 0.790735 11.1459 1.10826H11.1433Z" fill="url(#paint0_linear_%s)"/><defs><linearGradient id="paint0_linear_%s" x1="7.39992" y1="7.99947" x2="5.20783" y2="1.07424" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#E60C84"/><stop offset="1" stop-color="#FFCF4B"/></linearGradient></defs></svg>';
                                                                $inactiveIconSvg = '';
                                                            @endphp
                                                            @foreach ($activePackageDescription['features'] as $key => $feature)
                                                                @php
                                                                    $meta = [
                                                                        ...
                                                                        'type' => $feature->type,
                                                                        'title' => $feature->title,
                                                                        'title_position' => $feature->title_position,
                                                                        'value' => $feature->value
                                                                    ];
                                                                @endphp
                                                                @if ($meta['is_visible'])
                                                                    <div class="flex items-center text-color-14 dark:text-white text-15 font-normal font-Figtree gap-[9px]">
                                                                        @php $randomId = uniqid(); @endphp
                                                                        {!! sprintf($activeIconSvg, $randomId, $randomId) !!}
                                                                        <span>
                                                                            @if ($meta['type'] != 'number')
                                                                                {{ __($meta['title']) }}
                                        ...
                                        <div class="plan-root 6xl:gap-10 lg:gap-5 gap-6 lg:px-0 md:px-10 px-5 w-full flex flex-wrap justify-center {{count($packages) != 0 ? 'lg:mb-[140px] mb-[90px] 6xl:mt-[60px] mt-11' : ''}}">
                                            @foreach($packages as $key => $package)
                                                @foreach ($package['billing_cycle'] as $billing_cycle => $value)
                                                    ...
                                                    @php
                                                        // Reorder features based on predefined order
                                                        $orderedFeatures = [];
                                                    ...
                                                    <div  class="{{ $package['parent_class'] }} plan-parent plan-{{ $billing_cycle }} {{ ($hasMonthlyBilling && $billing_cycle == 'monthly') || (!$hasMonthlyBilling && $loop->first) ? '' : 'hidden' }} disable-gradient-border">
                                                        <div class="rounded-[30px] border border-color-89 dark:border-color-DFF bg-white dark:bg-color-14 pt-6 pb-5 sub-plan-rtl dark:bg-color-29 single-plan-container" data-supported-features="{{ json_encode($supportedFeatures, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) }}"> 
                                                            ...
                                                            <div class="features-list flex flex-col gap-[18px] mt-6 6xl:pl-11 lg:pl-5 pl-8 pr-4">
                                                                @if (!empty($supportedFeatures))
                                                                    @include('user.includes.partials.feature-highlights-tooltip', ['supportedFeatures' => $supportedFeatures])
                                                                @endif
