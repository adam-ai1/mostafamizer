...
class AiPersonaService
{
    ...
    public function __construct()
    {
        if (! is_null(request('provider'))) {
            $this->aiProvider = AiProviderManager::isActive(request('provider'), 'aipersona');
        }
    }
    ...
    public function validation()
    {
        ...
        manageProviderValues(request('provider'), 'model', 'aipersona');

        $validation = $this->aiProvider->getCustomerValidationRules('AiPersonaDataProcessor');
    ...
    {
        $videoResponse =  $this->aiProvider->generatePersona($requestData);
        ...
        try {
            ...
            $voice = Voice::where('voice_name', $requestData['options']['voice_id'])->first();

            $requestData['options']['avatar_name'] = $avatar->name;
            $requestData['options']['voice_name'] = "{$voice->name} (" . ucFirst($voice->language_code) . ") (" . ucFirst($voice->gender) . ")";

            unset($requestData['options']['avatar_id'], $requestData['options']['voice_id']);
    ...
    {
       return Archive::with('user', 'childs', 'metas')->where('id', $id)->where('type', 'ai_persona_chat')->first();
    }
    ...
    /**
     * Retrieves the latest avatars of the given type and provider.
     *
     * @param string $type The type of avatar to retrieve. Defaults to 'ai_persona'.
     * @param string $provider The provider of the avatar to retrieve. Defaults to 'heygen'.
     * @param array $options Additional options to filter the results. Valid options include 'custom'.
     * @return \Illuminate\Database\Eloquent\Builder|\Illuminate\Database\Eloquent\Collection
     */
    public function latestAvatars($type = 'ai_persona', $provider = 'heygen', $options = [])
    {
        return Avatar::with(['metas', 'user'])
            ->join('avatar_metas', 'avatars.id', '=', 'avatar_metas.owner_id')
            ->where('avatar_metas.key', 'image_url')
            ->when(isset($options['custom']), function ($query) use ($options) {
                $query->where('avatar_metas.key', 'custom')->where('avatar_metas.value', $options['custom']);
            })
            ->where([
                'avatars.type' => $type,
    ...
    public function latestVoices($type = 'ai_persona', $provider = 'heygen')
    {
        return Voice::with('metas')->where([
                'type' => $type,
                'providers' => $provider,
                'status' => 'Active'
            ]);
    ...
    {
        if (! $this->aiProvider) {
            throw new Exception(__(':x provider is not available for the :y. Please contact the administration for further assistance.', ['x' => ucfirst(request('provider')), 'y' => ucfirst(request('feature'))]));
        }
        ...
        try {
            ...
            $this->aiProvider->processStoreData($arrayData, $response->response);
