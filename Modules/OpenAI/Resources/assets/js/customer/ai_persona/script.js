...
$(document).on('submit', '#ai-persona-form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        beforeSend: function (xhr) {
            ...
            $('#ai-persona-generation').attr('disabled', 'disabled');
            xhr.setRequestHeader('Authorization', 'Bearer ' + ACCESS_TOKEN);
        },
        data: data,
        success: function(response) {
            if (response.message) {
                errorMessage(response.message, 'ai-persona-generation');
                return true;
            }
        ...
        complete: () => {
            ...
            $('#ai-persona-generation').removeAttr('disabled');
        ...
        error: function(response) {
            ...
            try {
                ...
                errorMessage(jsLang('Invalid server response'), 'ai-persona-generation');
                return true;
            }
            ...
            if (message) {
                errorMessage(message, 'ai-persona-generation');
                return true;
            }
         }
    });
...
function checkScrollIfAtEnd(contentContainer, listSelector, type) {
    ...
    if ((scrollPosition + clientHeight >= scrollHeight) && pageNumber != 0 && pageNumber.length != 0 && checked) {
        checked = false;
        ...
        doAjaxprocess(
            SITE_URL + '/' + 'user/ai-persona?type=' + type + '&' + type + '_page=' + pageNumber,
            {},
            'get',
            'json'
...
function searchItems(element, selector, type) {
    ...
    doAjaxprocess(
        SITE_URL + '/' + 'user/ai-persona?type=' + type + '&name=' + searchText.trim(),
        {},
        'get',
        'json'
    ).done(function (response) {
