...
trait TopViewApiTrait
{

    public function aiKey()
    {
        $apiKey = config('aiKeys.TOPVIEW.API_KEY');

        if (empty($apiKey)) {
            throw new \Exception(__("There's an issue with the :x API key. Kindly reach out to the administration for assistance.", ['x' => __('Top View')]), Response::HTTP_UNAUTHORIZED);
        }
    ...
    {
        $apiKey = config('aiKeys.TOPVIEW_API_ID.API_KEY');

        if (empty($apiKey)) {
            throw new \Exception(__("There's an issue with the :x API ID. Kindly reach out to the administration for assistance.", ['x' => __('Top View')]), Response::HTTP_UNAUTHORIZED);
        }
