...
$(document).on('submit', '.influencer-avatar-form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        beforeSend: function (xhr) {
            ...
            $('#influencer-generation').attr('disabled', 'disabled');
            xhr.setRequestHeader('Authorization', 'Bearer ' + ACCESS_TOKEN);
        },
        data: data,
        success: function(response) {
            if (response.message) {
                errorMessage(response.message, 'influencer-generation');
                return true;
            }
            ...
            if (response.data) {
                ...
                var containers = $('.ai-persona-list .video-container');
        ...
        complete: () => {
            ...
            $('#influencer-generation').removeAttr('disabled');
        ...
        error: function(response) {
            ...
            try {
                ...
                errorMessage(jsLang('Invalid server response'), 'influencer-generation');
                return true;
            }
            ...
            if (message) {
                errorMessage(message, 'influencer-generation');
                return true;
            }
         }
    });
...
function checkScrollIfAtEnd(contentContainer, listSelector, type) {
    ...
    if ((scrollPosition + clientHeight >= scrollHeight) && pageNumber != 0 && checked) {
        checked = false;
        ...
        doAjaxprocess(
            SITE_URL + '/' + 'user/influencer-avatar?type=' + type + '&' + type + '_page=' + pageNumber,
            {},
            'get',
            'json'
...
function renderSidebarAndOptions(parentDiv, items, type) {
    ...
    items.forEach(item => {
        ...
        if (type == 'voice') {
            const label = `${item.name} (${item.language_code}) (${item.gender})`;

            sidebarHTML += `
                <div class="dropdown-menu-item" data-value="${item.voice_name}" data-audio="${item.file_name}">
                    ${label}
                </div>
            `;

            optionsHTML += `
                <option value="${item.voice_name}" data-audio="${item.file_name}">
                    ${label}
                </option>
            `;
        }
    ...
    if (type == 'voice') {
        $('#voice_id').append(optionsHTML);

        parentDiv.off('click', '.dropdown-menu-item').on('click', '.dropdown-menu-item', function () {
            const $dropdown = $(this).closest('.dropdown');
            const selected = $dropdown.find('.dropdown-select')[0];
            const select = $dropdown.prev('select')[0];
            const menu = $dropdown.find('.dropdown-menu')[0];
            setSelected.call(this, selected, select, menu);
        });
    }
...
function searchItems(element, selector, type) {
    ...
    doAjaxprocess(
        SITE_URL + '/' + 'user/influencer-avatar?type=' + type + '&name=' + searchText.trim(),
        {},
        'get',
        'json'
    ).done(function (response) {
...
$(document).ready(function () {
    ...
    options.on('click', function (e) {
        ...
        const onchangeFunc = $this.data('onchange');

        selected.text(text);
        dropdown.addClass('hidden');
        if (onchangeFunc && typeof window[onchangeFunc] === 'function') {
            window[onchangeFunc].call(this, this); // Call the function with "this" as the element
        }
    });
...
function setAvatar(element) {
    ...
    const input = dropdown.find('input[type="text"].hidden')
    input.val($(element).data('value'));

    const parentDiv = $('.avatar-list');
    const type = 'avatar';
    parentDiv.empty();
    ...
    doAjaxprocess(
        SITE_URL + '/' + 'user/influencer-avatar?type=' + type,
        {
            source: $(element).data('value'),
        },
        'get',
        'json'
    ).done(function (response) {
