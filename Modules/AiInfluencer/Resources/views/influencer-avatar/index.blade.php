...
@section('page_title', __('Influencer Avatar'))
@section('css')
    ...
    <section
        ...
        <div
            ...
            <div
                ...
                <p class="text-color-14 text-24 font-semibold font-RedHat dark:text-white"> {{ __('Influencer Avatar') }} </p>
                <div class="flex flex-wrap justify-between items-center gap-5">
                    <p class="text-color-89 text-[13px] leading-5 font-medium font-Figtree mt-2">
                        {{ __(':x create studio-quality videos with AI Persona and voiceovers in 130+ languages. It’s as easy as making a slide deck.', ['x' => preference('company_name')]) }}
                    </p>
                    <div>
                ...
                <form id="ai-persona-form" class="influencer-avatar-form">
                    <section class="bg-white dark:bg-color-3A rounded-xl mt-5 md:px-6 px-4 pb-5">
                        ...
                        <div
                            ...
                            <label class="font-medium">{{ __('Provider') }}</label>
                            <select
                        ...
                        <div class="pt-6">
                            <div class="flex justify-start flex-wrap gap-4 items-center text-speech">
                                @foreach ($avatarOptions as $option)
                                    <div class="w-[275px] font-Figtree text-14 text-color-14 dark:text-white flex flex-col gap-1.5">
                                        <label class="font-medium"> {{ $option['label'] }} </label>
                                        
                                        <div class="relative">
                                            <button 
                                                id="select-btn"
                                                class="w-full mt-[3px] gap-2 py-2 px-3 h-12 text-14 dark:text-white dark:bg-[#333332] bg-white border-color-89 dark:border-color-47 border rounded-xl focus:border-blue-400 focus:outline-none text-left flex justify-between items-center">
                                                <span id="selected">{{ __('Select an option') }}</span>
                                                <span class="w-4 h-4">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                                        <g clip-path="url(#clip0_9797_5972)">
                                                            <path d="M4.2925 5L8.5 8.7085L12.7075 5L14 6.1417L8.5 11L3 6.1417L4.2925 5Z" fill="#898989"></path>
                                                        </g>
                                                        <defs>
                                            ...
                                            <div id="dropdown" class="absolute z-10 w-full mt-1 dark:text-white dark:bg-[#333332] bg-white border-color-89 border rounded-xl shadow-lg hidden">
                                                <input class="hidden" type="text" name="{{ $option['name'] }}" value="" id="{{ $option['name'] }}">
                                                @foreach ($option['value'] as $value)
                                                    <div class="option px-3 py-2 hover:bg-gray-100 cursor-pointer" data-value="{{ strtolower(str_replace(' ', '-', $value)) }}" data-id="{{ $value }}" @if(isset($option['onChange'])) data-onchange="{{ $option['onChange'] }}" @endif >
                                                        {{ ucwords($value) }}
                                                    </div>
                                                @endforeach
                                            </div>
                                ...
                                @if(count($avatars) > 0)
                                    <div class="font-normal font-Figtree text-14 text-color-14 dark:text-white flex flex-col gap-1.5">
                                        <label class="font-medium">{{ __('Avatar Name') }}</label>
                                        <div class="relative">
                                            <a
                                ...
                                @if(count($voices) > 0)
                                    <div class="flex justify-start flex-wrap gap-4 items-center">
                                        <div
                                            class="font-normal custom-dropdown-arrow text-14 text-color-14 dark:text-white flex flex-col gap-1.5">
                                            <div class="flex gap-2 justify-start items-center">
                                                <label class="font-medium">{{ __('Voice Selection') }}</label>
                                            </div>
                                            <section class="section wrapper wrapper-section">
                                                <div class="container wrapper-column">
                                                    <div class="form-group relative flex justify-between items-center w-[275px] md:w-[372px] text-[14px] leading-6 font-normal text-color-14 dark:text-white bg-white rounded-xl dark:bg-[#333332] m-0 border-color-89 border gap-2 py-2 px-3 dark:border-color-47 cursor-pointer h-12 voice-container"
                                                        data-next-page-url="{{ $voices->nextPageUrl() }}">
                                                        <select id="voice_id" name="voice_id"
                                                            class="select-audio dropdown w-[275px] md:w-[372px] p-4 mx-auto border border-color-89 dark:border-color-47 rounded-lg bg-white dark:bg-color-29 z-50">
                                                            @foreach ($voices as $option)
                                                                <option value="{{ $option->voice_name }}" data-audio="{{ $option->file_name }}">
                                                                    {{ $option->name }} ({{ $option->language_code }})({{ ucFirst($option->gender) }})
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                        ...
                                        <div
                                            class="player mt-6 bg-color-F6 dark:bg-[#292929] text-[#292929] dark:text-white rounded-md flex justify-center items-center w-10 h-10">
                                            <audio id="audio4" class="hidden"></audio>
                                            <i class="fa fa-play play" aria-hidden="true"></i>
                                            <i class="fa fa-pause pause !hidden" aria-hidden="true"></i>
                                        </div>
                                    </div>
                                @endif

                                @if(count($additionalOptions) > 0)
                                    <div class="mt-5 flex gap-1.5 justify-start items-center cursor-pointer" id="advanceToggle">
                            ...
                            @if(count($additionalOptions) > 0)
                                <div class="hidden mt-6" id="advanceContent">
                                    <div class="flex justify-start flex-wrap items-center gap-4 text-speech">
                                        @foreach ($additionalOptions as $field)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-5 ai-persona-list">
...
@section('js')
    ...
    <script src="{{ asset('Modules/AiInfluencer/Resources/assets/js/influencer_avatar/custom-dropdown.min.js') }}"></script>
    <script src="{{ asset('Modules/AiInfluencer/Resources/assets/js/influencer_avatar/script.min.js') }}"></script>
