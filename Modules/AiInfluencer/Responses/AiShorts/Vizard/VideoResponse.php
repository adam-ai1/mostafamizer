...
namespace Modules\AiInfluencer\Responses\AiShorts\Vizard;

use Exception;
use Modules\AiInfluencer\Contracts\Responses\AiShorts\VideoResponseContract;
...
class VideoResponse implements VideoResponseContract
{
    ...
    {
        if ($this->response['body']['code'] == 4001) {
            return $this->handleException(__("There's an issue with the :x API key. Kindly reach out to the administration for assistance.", ['x' => 'Vizard']));
        }
        
        if ($this->response['body']['code'] != 2000) {
            $message = $this->response['body']['errMsg'] ?? __('Something went wrong, please try again.');
    ...
    {
        return $this->videoId = $this->response['body']['projectId'];
    }
