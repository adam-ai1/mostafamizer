...
use Modules\AiInfluencer\Contracts\Responses\UrlToVideo\FetchVideoResponseContract;
...
class FetchVideoResponse implements FetchVideoResponseContract
{
    ...
    public function process()
    {
        ...
        if (empty($this->response['body']['result']['exportVideos'])) {
            return $this->handleException(__('No videos found for the provided video ID.'));
        }

        foreach ($this->response['body']['result']['exportVideos'] as $videoUrl) {
            $this->videos[] = file_get_contents($videoUrl['videoUrl']);
        }
