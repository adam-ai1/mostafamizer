...
namespace Modules\HeyGen\Traits;
use Illuminate\Http\Response;
use GuzzleHttp\Client;

trait HeyGenApiTrait 
{

    public function aiKey()
    {
        $apiKey = config('aiKeys.HEYGEN.API_KEY');

        if (empty($apiKey)) {
            throw new \Exception(__("There's an issue with the :x API key. Kindly reach out to the administration for assistance.", ['x' => __('HEYGEN')]), Response::HTTP_UNAUTHORIZED);
        }
