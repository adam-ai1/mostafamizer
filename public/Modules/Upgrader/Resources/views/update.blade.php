    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-block text-center mt-3">
                ...
                <span class="text-muted">{{ __('Current version: ') }}</span> <span class="badge badge-info">{{ config('artifism.file_version', env('VOXCRAFT_STUDIO_VERSION', '1.0.0')) }}</span>
                <div class="row justify-content-center">
                ...
                <div class="d-flex justify-content-start alert alert-secondary mt-3">
                    <ul>
                        <li>{{ __('Make sure your server has matched with all requirements.') }} <a href="{{ route('systemInfo.index') }}" target="_blank">{{ __('Check Here') }}</a>
                                                ...
                                                <li>{{ __('Download latest version voxcraft Studio from codecanyon.') }}  <a href="https://codecanyon.net/downloads" target="_blank"><i class="feather icon-external-link"></i> {{ __('Click here') }}</a></li>
