...
function setAttribute(data)
{
    ...
    const isVideo = videoTypes.includes(data.data('type'));
    $('.additional-options').toggleClass('hidden', isVideo);
    
    $('.variant-url').toggleClass('hidden', !isVideo ? (data.data('type') === 'productshot_variant') : false);
    ...
    if (withoutPrompt.includes(data.data('type'))) {
    ...
    if (isFavorite === true || isFavorite == 'true') {
        var favorite = `<div class="favorite-icon relative md:w-[30px] md:h-8 w-7 h-7 flex items-center m-auto justify-center rounded-lg bg-white wishlist-border">
        ...
        var favorite = `<div class="favorite-icon relative md:w-[30px] md:h-8 w-7 h-7 flex items-center m-auto justify-center rounded-lg delete-image-bg border border-color-47">
    ...
    for (const [key, value] of Object.entries(filteredData)) {
        htmlContent += `
                <div class="relative">
                    ...
                    <p class="tooltips font-Figtree leading-6 font-base text-color-14 dark:text-white font-semibold break-words line-clamp-double ${key} ${['negative_prompt', 'description'].includes(key) ? 'cursor-pointer' : ''}">${value}
                    ${['negative_prompt', 'description'].includes(key) ? `
...
function fetchImageData(slug) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.variant-options-slider').toggleClass('hidden', !data.data.variants.length >= 2);
            ...
            if (data.data.variants.length) {
                data.data.variants.forEach(function (item) {
                    if (videoTypes.includes(item.type)) {
                        variantImage += `<div class="swiper-slide cursor-grab !w-[80px] h-full rounded-lg old-img variant-images">
                            <video class="mx-auto w-[68px] h-[68px] rounded-[10px] object-cover myVideos" data-original="${item.video_url}" src="${item.video_url}" download="" />
                        </div>`
                    } else {
                        variantImage += `<div class="swiper-slide cursor-grab !w-[80px] h-full rounded-lg old-img variant-images">
                ...
                data.data.relatedImages.forEach(function (item) {
                    function ucFirst(str) {
                        if (!str) return '';
                        str = String(str).replace(/[-_]+/g, ' ').trim();
                        return str.charAt(0).toUpperCase() + str.slice(1).toLowerCase();
                    }
                    const optionsData = Object.entries(item.generation_options || {})
                        .map(([key, value]) => {
                            const safeValue = ucFirst(value);
                            return `data-advanced_${key}="${safeValue}"`;
                        })
                        .join(" ");
...
function filterBy(selectElement, type) {
    const value = selectElement.value;

    const newFilterType = type === 'orderBy' ? 'type' : 'orderBy';
    const newTypeVlaue = $('#' + newFilterType).val();
    ...
    doAjaxprocess(
        SITE_URL + '/' + 'user/gallery?' + type + '=' + value + '&' + newFilterType + '=' + newTypeVlaue,
        {},
        'get',
        'json'
    ).done(function (response) {
        var sidebarHTML = dataMapper(response);
        $('.fetch-skeleton').hide();
...
function dataMapper(response) {
    var html = response.items.map(function (item) {
        const generationOptions = item.generation_options || {};

        let advancedAttributes = '';
        for (const [key, value] of Object.entries(generationOptions)) {
            if (key === 'file') continue;
            advancedAttributes += ` data-advanced_${key}="${value}"`;
        }
        return `
            ...
            <div class="img-content bg-white img-grow md:rounded-lg rounded relative download-gallery-image-container download-image-container">
                <div class="tab-content-${item.id}">
                    ...
                    <div class="gallery-image-hover-overlay image-modal-button" 
                        ...
                        data-slug="${item.slug}" 
                        data-name="${item.title}" 
                        data-promt="${item.type == 'video' ? '' : item.title}"
                        data-type="${item.type}"
                        data-id="${item.id}" 
                        data-source="${videoTypes.includes(item.type) ? item.video_url : item.url}" 
                    ...
                    <div class="absolute top-0 ltr:left-0 rtl:right-0">
                        <div class="flex justify-end items-center gap-2 ltr:md:mr-2 ltr:mr-1 rtl:md:ml-2 rtl:ml-1">
                            <div class="image-download-button  mt-3 md:ml-2 ml-1 bg-color-14">
                                <a href="javascript: void(0)" class="relative md:w-[30px] md:h-8 w-7 h-7 flex items-center m-auto justify-center rounded-lg delete-image-bg border border-color-47 modal-toggle image-tooltip-delete gallery-dlt" id="${item.id}" type="${item.type}">
                            ...
                            ${
                                videoTypes.includes(item.type)
                                ? `
                                <div class="image-download-button mt-3 ltr:md:mr-2 ltr:mr-1 rtl:md:ml-2 rtl:ml-1 bg-color-14">
                                    <a href="javascript:void(0)"
                                    class="relative md:w-[30px] md:h-8 p-1 w-7 h-7 flex items-center justify-center m-auto rounded-lg delete-image-bg border border-color-47">
                                    <svg class="md:w-[30px] md:h-8 w-7 h-7" xmlns="http://www.w3.org/2000/svg" height="512pt" viewBox="-31 0 512 512" width="512pt">
                                    <g fill-rule="evenodd">
                                    <path fill="white" 
                                    ...
                                    </path><path fill="white" d="m303.921875 405.113281c0-3.574219-1.90625-6.875-5-8.660156l-87.855469-50.722656c-3.09375-1.785157-6.90625-1.785157-10 0-3.09375 1.785156-5 5.085937-5 8.660156v101.445313c0 3.570312 1.90625 6.871093 5 8.65625 1.546875.894531 3.273438 1.34375 5 1.34375 1.726563 0 3.453125-.449219 5-1.34375l87.855469-50.71875c3.09375-1.785157 5-5.085938 5-8.660157zm-87.855469 33.402344v-66.804687l57.855469 33.402343zm0 0"></path><path d="m234.773438 492c-5.507813 0-10 4.492188-10 10s4.492187 10 10 10c5.511718 0 10-4.492188 10-10s-4.488282-10-10-10zm0 0">
                                    </path></g></svg>
                                    </a>
                                </div>
                                `
                    ...
                    <div class="absolute top-0 right-0">
                        <div class="flex justify-end items-center gap-2 mt-3 md:mr-2 mr-1">
                            <div class="image-download-button delete-image-bg">
                                <a class="file-need-download relative md:w-[30px] md:h-8 w-7 h-7 flex items-center m-auto justify-center rounded-lg border border-color-47 image-tooltip-download" href="${videoTypes.includes(item.type) ? item.video_url : item.url}" download="${item.title}">
                                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div class="image-download-button modal-not-open delete-image-bg">
                                <a href="javascript: void(0)" class="favorite-image-${item.id}" onclick="imageToggle(this)" data-image-id="${item.id}" data-is-favorite="${item.is_favorite}">
                                    ${item.is_favorite ? `
                                        <div class="relative tooltips md:w-[30px] md:h-8 w-7 h-7 flex items-center m-auto justify-center rounded-lg bg-white wishlist-border">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M9 4.5L6.5 3L3.5 4L2.5 6V8.5L9 15L9.5 14.5L12 11.5L14.5 9.5L15.5 7.5L14.5 4.5L11.5 3L9 4.5Z" fill="#E22861"/>
                                                <path d="M9.00077 3.39692C9.85769 2.62982 10.9759 2.22007 12.1256 2.25187C13.2752 2.28368 14.3691 2.75462 15.1823 3.56792C15.9948 4.38031 16.4658 5.47273 16.4987 6.62123C16.5316 7.76974 16.1239 8.88733 15.3593 9.74492L8.99927 16.1139L2.64077 9.74492C1.87521 8.88689 1.46717 7.76834 1.50042 6.61891C1.53367 5.46948 2.00569 4.37638 2.81956 3.56404C3.63344 2.7517 4.72742 2.28175 5.87691 2.25067C7.02641 2.21959 8.14419 2.62974 9.00077 3.39692ZM14.1203 4.62767C13.5785 4.08626 12.85 3.77273 12.0843 3.75139C11.3187 3.73005 10.5739 4.00253 10.0028 4.51292L9.00152 5.41142L7.99952 4.51367C7.43077 4.00509 6.68961 3.73232 5.92687 3.75086C5.16412 3.7694 4.43709 4.07787 3.89373 4.61349C3.35037 5.14911 3.03151 5.87163 3.00202 6.63404C2.97254 7.39644 3.23465 8.14143 3.73502 8.71742L9.00002 13.9907L14.265 8.71817C14.7633 8.14472 15.0254 7.4036 14.9986 6.6444C14.9717 5.8852 14.6578 5.16446 14.1203 4.62767Z" fill="#E22861"/>
                                            </svg> 
                                    ...
                                    ` : `
                                        <div class="relative tooltips md:w-[30px] md:h-8 w-7 h-7 flex items-center m-auto justify-center rounded-lg border border-color-47">
                                            <svg width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M8.00077 1.39692C8.85769 0.629819 9.97588 0.220071 11.1256 0.251875C12.2752 0.283678 13.3691 0.754618 14.1823 1.56792C14.9948 2.38031 15.4658 3.47273 15.4987 4.62123C15.5316 5.76974 15.1239 6.88733 14.3593 7.74492L7.99927 14.1139L1.64077 7.74492C0.875208 6.88689 0.467169 5.76834 0.500419 4.61891C0.533668 3.46948 1.00568 2.37638 1.81956 1.56404C2.63344 0.751699 3.72742 0.281748 4.87691 0.250669C6.02641 0.21959 7.14418 0.629741 8.00077 1.39692ZM13.1203 2.62767C12.5785 2.08626 11.85 1.77273 11.0843 1.75139C10.3187 1.73005 9.57389 2.00253 9.00277 2.51292L8.00152 3.41142L6.99952 2.51367C6.43077 2.00509 5.68961 1.73232 4.92687 1.75086C4.16412 1.7694 3.43709 2.07787 2.89373 2.61349C2.35037 3.14911 2.03151 3.87163 2.00202 4.63404C1.97254 5.39644 2.23465 6.14143 2.73502 6.71742L8.00002 11.9907L13.265 6.71817C13.7633 6.14472 14.0254 5.4036 13.9986 4.6444C13.9717 3.8852 13.6578 3.16446 13.1203 2.62767Z" fill="white"/>
                                            </svg>
                                        </div>
...
function video() {
    $('.video-container .myVideos').each(function () {
        const $video = $(this);
        const $container = $video.closest('.video-container'); // assuming you wrap each video+buttons together
        const $playPauseBtn = $container.find('.playPauseBtn');
        const $playIcon = $container.find('.playIcon');
        const $pauseIcon = $container.find('.pauseIcon');
        const $progressContainer = $container.find('.progress-container');
        const $progressBar = $container.find('.progress-video-bar');
