...
@section('page_title', __('View Voiceover'))
@section('content')
            ...
            <div>
                ...
                <div class="bg-white dark:bg-color-3A rounded-xl lg:p-6 p-4">
                    ...
                    <div class="grid xl:grid-cols-2 mt-5">
                        <div class="mr-8">
                            <div class="grid grid-cols-2 gap-[100px]">
                                <div>
                                    <p class="font-Figtree text-color-89 text-[14px] leading-[22px] font-medium">{{ __('Provider') }}</p>
                                    <p class="font-Figtree text-color-14 dark:text-white text-base leading-6 font-semibold line-clamp-single">{{ ucfirst($audio->provider) }}</p>
                                </div>
                        ...
                        <div class="mt-4 xl:mt-0">
                            <div class="flex justify-between items-center">
                                ...
                                <p class="font-Figtree text-color-89 text-[12px] leading-[18px] font-medium">{{ __(':x Characters', ['x' => $audio->total_characters ]) }} </p>
                            </div>
                            <div class="mt-2 p-5 bg-[#F6F3F2] dark:border rounded-xl dark:bg-[#474746] dark:border-color-47">
                                <p class="word-breaks font-medium font-Figtree text-[15px] text-color-2C dark:text-white leading-[22px]"> {{ $audio->title }} </p>
                            </div>
                        </div>
                    </div>
                    <div class="grid xl:grid-cols-2 grid-cols-1 gap-8 mt-5">
                       <div>
                        ...
                        <div class="grid grid-cols-2 gap-[100px] mt-5">
                            @if (!empty($audio->generation_options))
                                @foreach (array_chunk($audio->generation_options, (int) ceil(count($audio->generation_options) / 2), true) as $column)
                                    <div>
                                        @foreach ($column as $key => $value)
                                            <div class="{{ !$loop->first ? 'mt-4' : '' }}">
                                                <p class="font-Figtree text-color-89 text-[14px] leading-[22px] font-medium">
                                                    {{ __(ucwords(str_replace('_', ' ', $key))) }}
                                                </p>
                                                <p class="font-Figtree text-color-14 dark:text-white text-base leading-6 font-semibold line-clamp-single">
                                                    @php
                    ...
                    <div class="mt-6 flex flex-wrap sm:gap-3 gap-2.5">
                        <a href="{{ $audio->googleAudioUrl() }}" download="{{ cleanedUrl(trimWords($audio->title, 30, '')) }}" class="file-need-download flex justify-center items-center gap-2 px-4 py-[9px] rounded-xl bg-color-14 down-img-btn">
