...
@section('content')
<!-- Main content -->
<div class="col-sm-12 list-container" id="audio-ads-list-container">
    <div class="card">
        ...
        <x-backend.datatable.filter-panel class="mx-1">
            ...
            <div class="col-md-2">
                <select class="select2 filter" name="ad_style">
                    <option value="">{{ __('All Styles') }}</option>
                    @foreach($adStyles as $key => $value)
                    <option value="{{ $key }}">{{ $value }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2">
                <select class="select2 filter" name="target_platform">
        ...
        <x-backend.datatable.table-wrapper class="user-list-wallet user-list-processing-message need-batch-operation"
            data-namespace="\Modules\OpenAI\Entities\AudioAd" data-column="id">
...
<script type="text/javascript">
    'use strict';
    var listContainer = "audio-ads-list-container";
</script>
