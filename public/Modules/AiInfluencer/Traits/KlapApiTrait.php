...
trait KlapApiTrait 
{
    ...
    {
        $apiKey = config('aiKeys.KLAP.API_KEY');

        if (empty($apiKey)) {
            throw new \Exception(__("There's an issue with the :x API key. Kindly reach out to the administration for assistance.", ['x' => 'KLAP']), Response::HTTP_UNAUTHORIZED);
        }
