...
use Modules\OpenAI\Services\TeamMemberService;
...
class InfluencerAvatarService
{
    ...
    private $type = 'influencer_avatar_chat';
    ...
    public function __construct()
    {
        if (! is_null(request('provider'))) {
            $this->aiProvider = AiProviderManager::isActive(request('provider'), 'influenceravatar');
        }
    }
    ...
    {
        if (! $this->aiProvider) {
            throw new Exception(__(':x provider is not available for the :y. Please contact the administration for further assistance.', ['x' => ucfirst(request('provider')), 'y' => __('Influencer Avatar')]));
        }
        
        // manageProviderValues(request('provider'), 'model', 'urltovideo');

        $validation = $this->aiProvider->getCustomerValidationRules('InfluencerAvatarDataProcessor');
    ...
    public function generate(array $requestData)
    {
        ...
        $videoResponse =  $this->aiProvider->generateInfluencerAvatar($requestData);
        $this->response = $this->aiProvider->retriveInfluencerAvatar($videoResponse->videoId);
