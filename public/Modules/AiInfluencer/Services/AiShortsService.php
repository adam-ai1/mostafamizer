...
class AiShortsService
{
    ...
    public function __construct()
    {
        if (! is_null(request('provider'))) {
            $this->aiProvider = AiProviderManager::isActive(request('provider'), 'aishorts');
        }
    }
    ...
    {
        if (! $this->aiProvider) {
            throw new Exception(__(':x provider is not available for the :y. Please contact the administration for further assistance.', ['x' => request('provider'), 'y' => __('Ai Shorts')]));
        }
        
        // manageProviderValues(request('provider'), 'model', 'aishorts');

        $validation = $this->aiProvider->getCustomerValidationRules('AiShortsDataProcessor');
    ...
    public function generate(array $requestData)
    {
        ...
        $videoResponse =  $this->aiProvider->generateAiShorts($requestData);
        $this->response = $this->aiProvider->retriveAiShorts($videoResponse->videoId);
