<div class="contacts-table table-content{{ isset($is_ajax) && $is_ajax ? '' : ' hidden' }}">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tr>
                    ...
                    <th class="text-start font-semibold px-6 py-5 text-xs uppercase tracking-wider">{{ __('Phone') }}</th>
                ...
                @forelse($contacts as $contact)
                    <tr data-contact-id="{{ $contact->id }}" class="border-t border-color-89/10 hover:bg-gradient-to-r hover:from-blue-50/50 hover:to-purple-50/50 dark:hover:from-blue-900/20 dark:hover:to-purple-900/20">
                        <td class="px-6 py-3 text-color-89 font-semibold whitespace-nowrap">#{{ $loop->index + 1 }}</td>
                        <td class="px-6 py-3 whitespace-nowrap text-color-14 dark:text-white">{{ $contact->name }}</td>
                        <td class="px-6 py-3 text-color-14 dark:text-white">+{{ $contact->phone }}</td>
                        <td class="px-6 py-3 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="editContact({{ $contact->id }})"
                                        type="button" aria-label="{{ __('Edit contact :x', ['x' => $contact->name] ) }}"
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.651 1.651a2.121 2.121 0 010 3.001l-9.193 9.193a2.121 2.121 0 01-1.061.577l-3.388.678a.75.75 0 01-.887-.887l.678-3.388a2.121 2.121 0 01.577-1.061l9.193-9.193a2.121 2.121 0 013.001 0z" />
                                    </svg>
                                </button>
                                <button type="button" data-target="deleteModal-{{ $contact->id }}" aria-label="{{ __('Delete contact :x', ['x' => $contact->name] ) }}"
                                ...
                                <!-- Delete Modal -->
                                <div id="deleteModal-{{ $contact->id }}" class="sweet-modal fixed inset-0 hidden bg-color-14/50 backdrop-blur-[4px] flex items-center justify-center z-50 transition-opacity duration-300 opacity-0 p-4">
                                    <div class="modalBox max-w-[600px] min-w-[300px] bg-white dark:bg-color-3A rounded-xl py-7 relative transform transition-all duration-300 scale-0 opacity-0">
                                        <form id="deleteContactForm-{{ $contact->id }}" class="px-7">
                                            @csrf
                                            @method('DELETE')
                                            ...
                                            <div class="flex justify-center gap-3 mt-6">
                                                <button type="submit"
                                                        class="deleteContactBtn w-full flex items-center justify-center rounded-[6px] text-sm text-center magic-bg hover:shadow-lg text-white px-5 py-2.5 font-medium hover:opacity-90 transition-all duration-200">
                                                    <span>{{ __('Delete') }}</span>
                ...
                @empty
                    <tr>
                        <td colspan="4">
                            ...
                            <p class="text-color-14 dark:text-white text-center font-medium font-Figtree text-20 mt-6">{{ __('No contacts found') }}</p>
                            <p class="text-center font-medium text-color-89 text-15 px-5 py-3 font-Figtree mt-3 md:w-[450px] mx-auto">{{ __('Looks like you did not add any contacts yet.') }}</p>
                        </td>
                    </tr>
                @endforelse
                </tbody>
    ...
    <!-- Pagination -->
    {{ $contacts->onEachSide(1)->links('site.layout.partials.pagination') }}
</div>